...
void PrintUsage(const Correct_Opt& opt) {
    ...
    if (opt.index == opt.correct){
        ...
        cout << "IMPORTANT: input (short) reads from the same pair MUST have the same name. /1 or /2 suffixes are not allowed." << endl << endl;
        ...
        if (opt.correct) {
            ...
            "   -s, --in-short                  Input short read file in fasta/fastq(.gz)" << endl <<
            "                                   List of input short read files (one file per line)" << endl <<
            "   -l, --in-long                   Input long read file to correct in fasta/fastq(.gz)" << endl <<
            ...
            "   -u, --in-unmapped-short         Input unmapped short read file in fasta/fastq(.gz)" << endl <<
            "                                   List of input unmapped short read files (one file per line)" << endl <<
            "   -a, --in-accurate-long          Input high quality long read file in fasta/fastq(.gz)" << endl <<
            ...
            "   -Q, --max-base-qual             Maximum base quality of input long reads (default: " << opt.max_qual << ")" << endl <<
            endl << "   > Optional with no argument:" << endl << endl <<
            "   -G, --gzip-out                  Output file is compressed with gzip" << endl <<
            "   -O, --force-io-order            Force same long read input/output order" << endl <<
            ...
            "   -2, --2nd-pass-only             Perform *only* the 2nd correction pass (default: " << (opt.pass2_only ? "true" : "false") << ")" << endl <<
            "   -F, --no-snp-correction         Disable SNP detection and correction" << endl <<
            "   -I, --no-graph-index            Disable graph index output" << endl << endl;
...
int parse_ProgramOptions(int argc, char **argv, Correct_Opt& opt) {
    ...
    const char* opt_string = "s:l:o:c:S:t:u:a:g:d:m:M:C:i:k:K:w:W:r:L:p:P:Q:12OFGv";
    ...
    static struct option long_options[] = {
        ...
        {"max-base-qual",           required_argument,  0, 'Q'},
        {"1st-pass-only",           no_argument,        0, '1'},
        {"2nd-pass-only",           no_argument,        0, '2'},
        {"force-correct-snp",       no_argument,        0, 'f'},
        {"force-io-order",          no_argument,        0, 'O'},
        {"no-snp-correction",       no_argument,        0, 'F'},
        {"no-graph-index",          no_argument,        0, 'I'},
        {"gzip-out",                no_argument,        0, 'G'},
    ...
    while ((c = getopt_long(argc - 1, argv + 1, opt_string, long_options, &option_index)) != -1) {
        ...
        switch (c) {
            ...
            case 'f':
                ...
                break;
            case 'O':
                opt.force_io_order = true;
                break;
            case 'G':
                opt.compress_out = true;
                break;
            case 'F':
                opt.force_no_snp_corr = true;
                break;
            case 'I':
                opt.force_no_graph_index = true;
                break;
            case 'v':
                opt.verbose = true;
                break;
...
bool check_ProgramOptions(Correct_Opt& opt) {
    ...
    if ((opt.trim_qual < 0) || (opt.trim_qual > opt.max_qual)){

        cerr << "Ratatosk::Ratatosk(): Quality score trimming threshold cannot be less than 0 or more than " << opt.max_qual << " (" << opt.trim_qual << " given)." << endl;
        ret = false;  
    }

    if (opt.k <= opt.small_k){

        cerr << "Ratatosk::Ratatosk(): Length of long k-mers for 2nd correction pass cannot be less than or equal to length of short k-mers for 1st correction pass (" << opt.k << " and " << opt.small_k << " given)." << endl;
        ret = false;  
    }
    ...
    if (opt.max_qual < 0){
        ...
        cerr << "Ratatosk::Ratatosk(): Maximum base quality cannot be less than 0" << endl;
        ret = false;  
    }
...
void writeCorrectedOutput(zstr::ofstream& out, const string& name_str, const string& seq_str, const string& qual_str, const int k, const int trim = 0){
...
void search(const CompactedDBG<UnitigData>& dbg, const Correct_Opt& opt, const bool long_read_correct, const Roaring* partitions, const pair<HapReads, HapReads>& hap_reads) {
    ...
    const bool compressOut = opt.compress_out && long_read_correct && (!opt.force_io_order || (opt.nb_threads == 1));
    ...
    zstr::ofstream zout(fn_out + ".gz", ios_base::out);
    ...
    if (opt.nb_threads == 1){
        ...
        while (fp.read(in_read, file_id)){ // For every read (a query)
            ...
            if (!long_read_correct && !in_qual.empty()) getStdQual(in_qual, opt.max_qual); // Set all quality score from 0 to 40
            ...
            if (long_read_correct){
                ...
                if (opt.compress_out) writeCorrectedOutput(zout, in_name, in_read, in_qual, k, (long_read_correct ? opt.trim_qual : 0));
                else writeCorrectedOutput(out, in_name, in_read, in_qual, k, (long_read_correct ? opt.trim_qual : 0));
            }
            else {
                ...
                writeCorrectedOutput(zout, in_name, in_read, in_qual, k, (long_read_correct ? opt.trim_qual : 0));
            }
                        ...
                        if (stop){
                            ...
                            if (!v_in_read.empty()){
                                ...
                                for (size_t i = 0; i < v_in_read.size(); ++i){
                                    ...
                                    if (!long_read_correct && !v_in_qual[i].empty()) getStdQual(v_in_qual[i], opt.max_qual); // Set all quality score from 0 to 40
                                ...
                                if (compressOut) {
                                    ...
                                    for (size_t i = 0; i < v_in_read.size(); ++i) writeCorrectedOutput(zout, v_in_name[i], v_in_read[i], v_in_qual[i], k, (long_read_correct ? opt.trim_qual : 0));
                                }
                                else {
    ...
    if (compressOut && (remove(fn_out.c_str()) != 0)) cerr << "Ratatosk::search(): Couldn't remove temporary file" << endl;
    if (!compressOut && (remove((fn_out + ".gz").c_str()) != 0)) cerr << "Ratatosk::search(): Couldn't remove temporary file" << endl;

    // File reordering -> makes sure output reads are in the same order as input reads. It's more IO but meh, what can I do...
    if ((!long_read_correct || opt.force_io_order) && (opt.nb_threads != 1)) { // If one thread used, output file is same order as input file so no reordering needed
        ...
        if (!long_read_correct || !opt.compress_out) {
        ...
        for (const auto p : v_block_w) {
            ...
            if (long_read_correct && opt.compress_out) zout.write(buffer, block_sz);
            else out.write(buffer, block_sz);
        }

        delete buffer;

        if (long_read_correct && opt.compress_out) zout.close();
        else outfile.close();
        ...
        if (!long_read_correct || !opt.compress_out) {
            ...
            if (rename(fn_tmp_out.c_str(), fn_out.c_str()) != 0) {

                cerr << "Ratatosk::search(): Reordering could not happen because file " << fn_tmp_out << " could not be renamed. Abort." << endl;
                exit(1);
            }

            if (remove((fn_out + ".gz").c_str()) != 0) cerr << "Ratatosk::search(): Couldn't remove temporary file" << endl;
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) PrintUsage(opt);
        ...
        if (parse == 1) PrintVersion();
            ...
            const string ext_reads = ".fastq";
            const string ext_graph = ".fasta.gz";
            const string ext_graph_index = ".bfi";
            const string ext_graph_data = ".rtsk";
            ...
            if (opt.pass1_only || !opt.pass2_only) { // Correction pass 1
                ...
                if (!hasIndex) {
                    ...
                    dbg.build(opt_pass1);
                    dbg.write(graph_long, opt_pass1.nb_threads, false, true, false, !opt_pass1.force_no_graph_index, true, opt_pass1.verbose);
                ...
                if (!dbg_empty){
                    ...
                    if (hasIndex) {
                        ...
                        opt_pass1.filename_ref_in = vector<string>(1, graph_long + ext_graph);
                        ...
                        addCoverage(dbg, opt_pass1, hapPass1.first, false, graph_long + ext_graph);

                        if (!opt_pass1.force_no_snp_corr) {

                            if (opt_pass1.verbose) cout << "Ratatosk::Ratatosk(): Adding SNPs candidates to graph (1/2)." << endl;

                            detectSNPs(dbg, opt_pass1);
                        }
                        else if (opt_pass1.verbose) cout << "Ratatosk::Ratatosk(): SNPs candidate detection is disabled (1/2)." << endl;
                    ...
                    if (opt.index) {
                        ...
                        dbg.write(graph_short, opt_pass1.nb_threads, false, true, false, !opt_pass1.force_no_graph_index, true, opt_pass1.verbose); // Write graph

                        writeGraphData(graph_short + ext_graph_data, dbg, opt_pass1.verbose); // Write UnitigData
                    }
            ...
            if (opt.pass2_only || !opt.pass1_only) {
                ...
                if (opt.pass2_only){
                    ...
                    opt_pass2.filename_seq_in.clear();
                    opt_pass2.filename_seq_in.insert(opt_pass2.filename_seq_in.end(), opt_pass2.filenames_helper_long_in.begin(), opt_pass2.filenames_helper_long_in.end());
                    opt_pass2.filename_seq_in.push_back(opt_pass1.filename_long_out + ext_reads);

                    opt_pass2.filenames_long_in.clear();
                    opt_pass2.filenames_long_in.push_back(opt_pass1.filename_long_out + ext_reads);

                    dbg.read(graph_long + ext_graph, opt_pass2.nb_threads, opt_pass2.verbose);

                    { // Remove long k-mer graph and unmapped reads temporary files from disk

                        if (!opt.pass1_only && !opt.pass2_only && (graph_long.length() != 0)) {

                            if (remove((graph_long + ext_graph).c_str()) != 0) cerr << "Ratatosk::Ratatosk(): Couldn't remove temporary file" << endl;
                            if (remove((graph_long + ext_graph_index).c_str()) != 0) cerr << "Ratatosk::Ratatosk(): Couldn't remove temporary file" << endl;
                        }
                ...
                if (!dbg_empty || opt.pass2_only){
                    ...
                    if (dbg.length() != 0) {
                        ...
                        if (hasData) {
                            ...
                            addCoverage(dbg, opt_pass2, hapPass2.second, true, graph_long + ext_graph);

                            if (!opt_pass2.force_no_snp_corr) {

                                if (opt_pass2.verbose) cout << "Ratatosk::Ratatosk(): Adding SNPs candidates to graph (2/2)." << endl;

                                detectSNPs(dbg, opt_pass2);
                            }
                            else if (opt_pass2.verbose) cout << "Ratatosk::Ratatosk(): SNPs candidate detection is disabled (2/2)." << endl;
                        ...
                        if (opt.index) {
                            ...
                            if (opt.pass2_only && !hasGraph) dbg.write(graph_long, opt_pass2.nb_threads, false, true, false, !opt_pass2.force_no_graph_index, true, opt_pass2.verbose); // Write graph

                            writeGraphData(graph_long + ext_graph_data, dbg, opt_pass2.verbose); // Write UnitigData
                        }
                        else { // correct
                        ...
                        copyFile(opt_pass2.filename_long_out + ext_reads, opt_pass2.filenames_long_in); // Graph is empty, LR cannot be corrected
                    }
                }
                else copyFile(opt_pass2.filename_long_out + ext_reads, opt.filenames_long_in); // Graph is empty, LR cannot be corrected
            }

            if (!opt.pass1_only && !opt.pass2_only) { // Clean up tmp files if correction was made all at once from the raw files

                if (graph_long.length() != 0){

                    if (remove((graph_long + ext_graph).c_str()) != 0) cerr << "Ratatosk::Ratatosk(): Couldn't remove temporary file" << endl;
                    if (remove((graph_long + ext_graph_index).c_str()) != 0) cerr << "Ratatosk::Ratatosk(): Couldn't remove temporary file" << endl;
                }

                if (!dbg_empty && (opt_pass1.filename_long_out.length() != 0) && (remove((opt_pass1.filename_long_out + ext_reads).c_str()) != 0)) cerr << "Ratatosk::Ratatosk(): Couldn't remove temporary file" << endl;
            }
        }
    }
}