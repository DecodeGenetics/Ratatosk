...
/*BlockedBloomFilter createBloomFilter(const CDBG_Build_opt& opt, const CompactedDBG<UnitigData>& dbg, const size_t nb_threads){

	BlockedBloomFilter bf(dbg.nbKmers(), opt.nb_bits_unique_kmers_bf);

	if (nb_threads == 1){

		for (const auto& um : dbg){

			const string s(um.referenceUnitigToString());

	        KmerHashIterator<RepHash> it_kmer_h(s.c_str(), s.length(), opt.k), it_kmer_h_end;
	        minHashIterator<RepHash> it_min(s.c_str(), s.length(), opt.k, opt.g, RepHash(), true);
	        ...
	        for (; it_kmer_h != it_kmer_h_end; ++it_kmer_h) {
	            ...
	            bf.insert(p_.first, it_min.getHash(), false);
	        }
		}
	}
	else {
	    ...
	    CompactedDBG<UnitigData>::const_iterator g_a = dbg.begin();

	    const CompactedDBG<UnitigData>::const_iterator g_b = dbg.end();
	    ...
	    for (size_t t = 0; t < nb_threads; ++t){
	        ...
	        workers.emplace_back(
	            ...
	            [&, t]{
	                ...
	                CompactedDBG<UnitigData>::const_iterator l_a, l_b;
	                ...
	                while (true) {

	                    {
	                        ...
	                        for (size_t i = 0; (i < chunk) && (g_a != g_b); ++i, ++g_a) {}
	                    ...
	                    while (l_a != l_b) {
							...
							const string s(l_a->referenceUnitigToString());
					        ...
					        for (; it_kmer_h != it_kmer_h_end; ++it_kmer_h) {
					            ...
					            bf.insert(p_.first, it_min.getHash(), true);
					        }
