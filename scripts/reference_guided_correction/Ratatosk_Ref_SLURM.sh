...
do
	...
	then
		...
		do
			...
			RATATOSK_CMD="Ratatosk correct -v -c ${MIN_NB_THREADS} -s ${NAME_SR_IN_FILE}.fa -l ${NAME_LR_IN_FILE}.fq -u ${NAME_SR_UNMAPPED_IN_FILE}.fa -o ${NAME_LR_OUT_FILE}"
...
CMD="${CMD} Ratatosk correct -v -c ${MAX_NB_THREADS} -s ${PREFIX_PATH_SEG}/sample_sr.fastq.gz -l ${PREFIX_PATH_SEG}/sample_lr_unknown.fq -a ${PREFIX_PATH_SEG}/sample_lr_map.fastq -o ${PREFIX_PATH_SEG}/sample_lr_unknown_corrected;" # Ratatosk
...
SLURM_OUT=$(sbatch -p ${PARTITION} -J Ratatosk_correctSegONT2 --dependency=afterok:${JOB_ID} --mem=250G --cpus-per-task=${MAX_NB_THREADS} -t 7-0:0 -o ${PREFIX_PATH_LOG}/correctSegONT2.slurm.out -e ${PREFIX_PATH_LOG}/correctSegONT2.slurm.err --wrap="${CMD}")
