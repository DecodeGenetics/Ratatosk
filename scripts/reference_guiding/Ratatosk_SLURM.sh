...
while [[ "$#" -gt 0 ]]; do
    case $1 in
		...
		-n|--slurm_part) PARTITION="$2" ; shift ;;
		-h|--help) PRINT_HELP=1 ;;
...
then
	...
	echo "Usage:"
	echo ""
	echo "bash $0 -c <MIN_NB_THREADS> -C <MAX_NB_THREADS> -r <REFRENCE_GENOME> -s <SHORT_READS_BAM> -l <LONG_READS_BAM> -o <OUTPUT_PATH> -p [SHORT_READS_PHASING] -P [LONG_READS_PHASING] -n [SLURM_PARTITION]"
	echo ""
	echo "<Mandatory>: "
	...
	echo ""
	echo "[Optional]: "
	echo "-n [SLURM_PARTITION]: Slurm partition (default: nomosix)."
...
SLURM_OUT=$(sbatch -p ${PARTITION} -J Ratatosk_extractBins --mem=24G --cpus-per-task=${MAX_NB_THREADS} -t 2-0:0 -o extractBins.slurm.log --wrap="${CMD}")
...
echo "#SBATCH --cpus-per-task=${MIN_NB_THREADS}" >> ${PREFIX_FILE_BIN}
...
SLURM_OUT=$(sbatch -p ${PARTITION} -J Ratatosk_correctSegONT2 --dependency=afterok:${JOB_ID} --mem=350G --cpus-per-task=${MAX_NB_THREADS} -t 7-0:0 -o correctSegONT2.slurm.log --wrap="${CMD}")
